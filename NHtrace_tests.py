...
{
call debug.DebugNL(("test3.v : 7 : f BuildRequest [<>][<>]",startDate,"_"))
    ...
    req->reqDateRange->startDate := ((startDate - leaway days) if leaway >= 1, startDate otherwise)    
    req->reqDateRange->endDate := ((startDate + leaway days) if leaway >= 1, startDate otherwise)    
call debug.DebugNL(("test3.v : 34 : ret BuildRequest [<>]","_"))
    return req    
}
    '''
out_test2='''
...
{
call debug.DebugNL(("test2.v : 18 : f ExtrasSearch [<>][<>][<>][<>]",companyNo,clientNo,"_",test2))
    ...
    {
call debug.DebugNL(("test2.v : 25 : f ExtrasSearch.innerfunc [<>][<>]",P_first,P_second))

        body := true
call debug.DebugNL(("test2.v : 28 : ret ExtrasSearch.innerfunc [<>]",body))
        return body 
    }

    
    if clientNo in (0, null) {    
call debug.DebugNL(("test2.v : 34 : ret ExtrasSearch [<>]","_"))
        return null    
    }
    
    if isInternet and startDate < today + 2 days {    
        raise ex.arg, "Invalid start date. Start date cannot be before " ^ today + 2 days    
call debug.DebugNL(("test2.v : 40 : ret ExtrasSearch [<>]","_"))
        return null    
    }
    ...
    call BldExtraDateBases(companyNo, clientNo)    
    if dateResultList->IsEmpty() {    
call debug.DebugNL(("test2.v : 47 : ret ExtrasSearch [<>]","_"))
        return null    
    }
    ...
    where F_test = test 
    {
call debug.DebugNL(("test2.v : 54 : ExtrasSearch.accomref select <>",""))
    ...
    res->dateList := dateResultList    
    res->extraList := extrares.BuildExtraResult(this)    
call debug.DebugNL(("test2.v : 63 : ret ExtrasSearch [<>][<>][<>]","_"true,true))
    return res,true,true
}
    '''
out_test5='''
    ...
    {
call debug.DebugNL(("test5.v : 7 : p RelatedRooms [<>]","_"))
        ...
        if !room->hasRelated {    
            ...
            order by F_rel_level    
            {
call debug.DebugNL(("test5.v : 15 : RelatedRooms.accomrel select <>",""))
                ...
                if (doInterconnecting in (AllRooms, NoInterconnecting) or    
                   (doInterconnecting in (InterconnectingOnly, StdPlusInterconnecting) and F_rel_level != 'I')) {    
                    ...
                    where accom.F_accom_no = accomrel.F_rel_accom_no    
                    {
call debug.DebugNL(("test5.v : 25 : RelatedRooms.accomrel.accom select <>",""))
...
out_test4='''
    ...
    where F_accomref_no=0
    {
call debug.DebugNL(("test4.v : 5 : accomref select <>",""))
    ...
    where F_accomref_no=0
    {
call debug.DebugNL(("test4.v : 13 : accomref update <>",""))
...
{
call debug.DebugNL(("test.v : 10 : p ExtrasSearch [<>][<>]",companyNo,clientNo))
    
    if clientNo in (0, null) {    
call debug.DebugNL(("test.v : 13 : ret ExtrasSearch ",))
        return      
    }    
    if isInternet and startDate < today + 2 days {    
        raise ex.arg, "Invalid start date. Start date cannot be before " ^ today + 2 days    
call debug.DebugNL(("test.v : 18 : ret ExtrasSearch ",))
        return    
    }    
    dateResultList := empty(dateres.DateResultList)    
    call BldExtraDateBases(companyNo, clientNo)    
    if dateResultList->IsEmpty() {    
call debug.DebugNL(("test.v : 24 : ret ExtrasSearch ",))
        return      
    }    
    let res := empty(result.Result)    
    res->dateList := dateResultList    
    res->extraList := extrares.BuildExtraResult(this)    
call debug.DebugNL(("test.v : 30 : ret ExtrasSearch ",))
    return      
...
request='''constant numberOfCompanies = 5    
...
--call promotions.Debug(Display(1))    
    ...
    {    
        ...
        if res = null or not(res->ValidResultSet(this)) {    
            ...
            /*let oldStartBase := this->yachtStartBase    
            let oldEndBase := this->yachtEndBase    
                
            if (res = null or not(res->ValidResultSet(this)))    
            and retRoute != true    
            and (oldStartBase != oldEndBase)    
            {    
                this->yachtStartBase := oldEndBase    
                this->yachtEndBase := oldStartBase    
                    
                reqDateRange->startDate := reqDateRange->startDate    
                reqDateRange->endDate := reqDateRange->endDate    
                ...
                -- need to add stopper in so does not get stuck in loop if no availability is found in either direction    
            }    
            */        
...
--display accomNo, res->accomDateList->elemCount, fromBase, toBase    
    ...
    {    
        ...
        } /* else { -- switch off for now    
                -- for normal LeBoat web and new availability we now also need to check numbers of pax    
                -- based on the following rules:    
                -- Use number of pax to return (a) where expected pax on boat >= pax up to pax + 3     
                        -- Use number of pax to return (b) extra boats where max pax covers requirement in (a)    
                            
                        if holidayType = holtype.Waterways and checkExpectedPax != null    
                        {    
                                let totPax := adultPax + childPax    
                        -- first check we are within the ranges    
                if accTypeTu->F_min_sale > totPax or totPax > accTypeTu->F_max_sale     
                {    
                                    return false    
                            }    
    
                                if (checkExpectedPax and accTypeTu->F_expected_pax > totPax + 3)     
                                or (!checkExpectedPax and accTypeTu->F_max_sale > totPax + 3)    
                    {    
                        return false    
                    }        
            }    
...
/*    
                                                        quick select from avail index F_key    
                                                        where avail.F_company_no = baseDate->companyNo    
                                                        and avail.F_accom_no = accom.F_accom_no    
                                                        and avail.F_base = baseDate->baseCode    
                                                        and avail.F_display != "N"    
                ...
                and pub.AvailAccessOk(avail.F_access)    
                                                        ...
                                                        {    
                                                            call ProcessRoom(baseDate->baseCode, baseDate, dateRes)    
                                                            stop    
                                                        }    
*/    
                                                    }    
...
{    
    ...
    resultTxt is text    
    resultTxt := ("List has <> elements", avbkRes->accomDateList->elemCount)    
    while tmpAccomDate != null {    
        let accomDate := cast(tmpAccomDate, avbk.AccomDateElement)    
        let dt, tm := avbkdt.DateAndTime(accomDate->startDT)    
        resultTxt := resultTxt^ "    " ^ (roomResult->accomNo if roomResult != null, yachtResult->accomNo otherwise)    
            ^ " Date " ^dt^ " Time " ^tm    
            ^ " From " ^accomDate->startBase^ " To " ^accomDate->endBase    
            ^ "            Before: Gap " ^avbkdt.DurationToDHM(accomDate->bfrGapDur)    
                ^ ". Delivery " ^accomDate->bfrDelDays    
                ^ " days. Before base " ^accomDate->bfrBase    
            ^ "            After:  Gap  " ^avbkdt.DurationToDHM(accomDate->afrGapDur)    
                ^ ". Delivery " ^accomDate->afrDelDays    
                ^ " days. After base " ^accomDate->afrBase    
            ^ ("            Singles already booked " ^accomDate->bookedSinglesPax if accomDate->bookedSinglesPax != null, null otherwise)    
            ^ ""    
...
'''
out_request='''constant numberOfCompanies = 5    
...
public class Request is    
{
    ...
    proc PopulateFromXML(inNode is xmlnodes.BaseNode)    
    {
call debug.DebugNL(("request.v : 101 : p Request->PopulateFromXML [<>]","_"))
        ...
        where holtype.F_code = holidayTypeCode    
        {
call debug.DebugNL(("request.v : 116 : Request->PopulateFromXML.holtype select <>",""))
    ...
    {
call debug.DebugNL(("request.v : 279 : p Request->PopulateFromBookXML [<>][<>][<>][<>]","_",adltsPax,kidsPax,infPax))
    ...
    {
call debug.DebugNL(("request.v : 325 : p Request->PopulateFromAccomref [<>][<>][<>][<>]","_",adltsPax,kidsPax,infPax))
    ...
    {
call debug.DebugNL(("request.v : 380 : p Request->SetExtraOptions [<>]","_"))
    ...
    public function SingleAccomReqd(yachtRequest is boolean) returns boolean    
    {
call debug.DebugNL(("request.v : 389 : f Request->SingleAccomReqd [<>]",yachtRequest))
...
call debug.DebugNL(("request.v : 412 : ret Request->SingleAccomReqd [<>]",singleAccom))
        return singleAccom    
    }
    ...
    public function SinglesReqd(yachtRequest is boolean) returns boolean    
    {
call debug.DebugNL(("request.v : 419 : f Request->SinglesReqd [<>]",yachtRequest))
...
call debug.DebugNL(("request.v : 442 : ret Request->SinglesReqd [<>]",singlesReqd))
        return singlesReqd    
    }
    ...
    {
call debug.DebugNL(("request.v : 453 : f Request->ExtrasSearch [<>][<>]",companyNo,clientNo))
    
        if clientNo in (0, null) {    
call debug.DebugNL(("request.v : 456 : ret Request->ExtrasSearch [<>]","_"))
            return null    
        }
    
        if isInternet and startDate < today + 2 days {    
            raise ex.arg, "Invalid start date. Start date cannot be before " ^ today + 2 days    
call debug.DebugNL(("request.v : 462 : ret Request->ExtrasSearch [<>]","_"))
            return null    
        }
        ...
        call BldExtraDateBases(companyNo, clientNo)    
        if dateResultList->IsEmpty() {    
call debug.DebugNL(("request.v : 469 : ret Request->ExtrasSearch [<>]","_"))
            return null    
        }
        ...
        res->dateList := dateResultList    
        res->extraList := extrares.BuildExtraResult(this)    
call debug.DebugNL(("request.v : 476 : ret Request->ExtrasSearch [<>]","_"))
        return res    
    }
    ...
    {
call debug.DebugNL(("request.v : 487 : p Request->BldExtraDateBases [<>][<>]",companyNo,clientNo))
        ...
        {
call debug.DebugNL(("request.v : 498 : p Request->BldExtraDateBases.BldExtraBases [<>][<>][<>][<>][<>]","_",companyNo,clientNo,startDate,dur))
            ...
            and baseref.F_duration = dur    
            {
call debug.DebugNL(("request.v : 508 : Request->BldExtraDateBases.BldExtraBases.accomref select <>",""))
                ...
                and accomref.F_duration = baseref.F_duration    
                {
call debug.DebugNL(("request.v : 523 : Request->BldExtraDateBases.BldExtraBases.accomref.accomref select <>",""))
        ...
        order by dateref.F_start_date ascending    
        {
call debug.DebugNL(("request.v : 546 : Request->BldExtraDateBases.accomref select <>",""))
    ...
    {
call debug.DebugNL(("request.v : 570 : f Request->XMLBookSearch [<>][<>]","_","_"))
    
        ------------------------------------------------------------------------------------------------------    
        procedure Product(base is string)    
        {
call debug.DebugNL(("request.v : 575 : p Request->XMLBookSearch.Product [<>]",base))
        ...
        on ex.once {    
            raise ex.arg, "Invalid holiday type code :" ^ holidayTypeCode    
call debug.DebugNL(("request.v : 592 : ret Request->XMLBookSearch [<>]","_"))
            return null    
        }
        ...
        where holtype.F_code = holidayTypeCode    
        {
call debug.DebugNL(("request.v : 599 : Request->XMLBookSearch.holtype select <>",""))
        ...
        if clubBases = null and yachtBases = null {    
            ...
            if !rv {    
                if closedBaseList != null and !closedBaseList->IsEmpty() {    
                    let res := empty(result.Result)    
                    res->closedBaseList := closedBaseList    
call debug.DebugNL(("request.v : 643 : ret Request->XMLBookSearch [<>]","_"))
                    return res    
                }
    
call debug.DebugNL(("request.v : 647 : ret Request->XMLBookSearch [<>]","_"))
                return empty(result.Result)    
            }
        ...
        -- restore the calling company environment.    
        gl_company_no := oldCompanyNo    
call debug.DebugNL(("request.v : 709 : ret Request->XMLBookSearch [<>]","_"))
        return res    
    }
    ...
    {
call debug.DebugNL(("request.v : 718 : f Request->AccomrefBookSearch [<>]","_"))
    
        ------------------------------------------------------------------------------------------------------    
        procedure Product(base is string)    
        {
call debug.DebugNL(("request.v : 723 : p Request->AccomrefBookSearch.Product [<>]",base))
        ...
        on ex.once {    
            raise ex.arg, "Invalid holiday type code :" ^ holidayTypeCode    
call debug.DebugNL(("request.v : 740 : ret Request->AccomrefBookSearch [<>]","_"))
            return null    
        }
        ...
        where holtype.F_code = holidayTypeCode    
        {
call debug.DebugNL(("request.v : 747 : Request->AccomrefBookSearch.holtype select <>",""))
        ...
        if clubBases = null and yachtBases = null {    
            ...
            if !rv {    
                if closedBaseList != null and !closedBaseList->IsEmpty() {    
                    let res := empty(result.Result)    
                    res->closedBaseList := closedBaseList    
call debug.DebugNL(("request.v : 791 : ret Request->AccomrefBookSearch [<>]","_"))
                    return res    
                }
    
call debug.DebugNL(("request.v : 795 : ret Request->AccomrefBookSearch [<>]","_"))
                return empty(result.Result)    
            }
        ...
        -- restore the calling company environment.    
        gl_company_no := oldCompanyNo    
call debug.DebugNL(("request.v : 857 : ret Request->AccomrefBookSearch [<>]","_"))
        return res    
    }
    ...
    returns result.Result with null    
    {
call debug.DebugNL(("request.v : 865 : f Request->FlightSearch ",))
    
        on ex.once {    
            raise ex.arg, "Invalid holiday type code :" ^ holidayTypeCode    
call debug.DebugNL(("request.v : 869 : ret Request->FlightSearch [<>]","_"))
            return null    
        }
    
        if isInternet and startDate < today + 2 days {    
            raise ex.arg, "Invalid start date. Start date cannot be before " ^ today + 2 days    
call debug.DebugNL(("request.v : 875 : ret Request->FlightSearch [<>]","_"))
            return null    
        }
        ...
        where holtype.F_code = holidayTypeCode    
        {
call debug.DebugNL(("request.v : 882 : Request->FlightSearch.holtype select <>",""))
        ...
        if clubBases = null and yachtBases = null {    
            ...
            if !rv {    
                if closedBaseList != null and !closedBaseList->IsEmpty() {    
                    let res := empty(result.Result)    
                    res->closedBaseList := closedBaseList    
call debug.DebugNL(("request.v : 914 : ret Request->FlightSearch [<>]","_"))
                    return res    
                }
 else {    
call debug.DebugNL(("request.v : 918 : ret Request->FlightSearch [<>]","_"))
                    return empty(result.Result)    
                }
        ...
        if flightCnt = 0 {    
            gl_company_no := oldCompanyNo    
call debug.DebugNL(("request.v : 948 : ret Request->FlightSearch [<>]","_"))
            return empty(result.Result)    
        }
        ...
        -- restore the calling company environment.    
        gl_company_no := oldCompanyNo    
call debug.DebugNL(("request.v : 957 : ret Request->FlightSearch [<>]","_"))
        return res    
    }
    ...
    returns result.Result with null    
    {
call debug.DebugNL(("request.v : 966 : f Request->AvailSearch ",))
    
    
        let oldLeeway := leaway    
        on ex.once {    
            raise ex.arg, "Invalid holiday type code :" ^ holidayTypeCode    
call debug.DebugNL(("request.v : 972 : ret Request->AvailSearch [<>]","_"))
            return null    
        }
    
        if isInternet and startDate < today + 2 days {    
            raise ex.arg, "Invalid start date. Start date cannot be before " ^ today + 2 days    
call debug.DebugNL(("request.v : 978 : ret Request->AvailSearch [<>]","_"))
            return null    
        }
        ...
        where holtype.F_code = holidayTypeCode    
        {
call debug.DebugNL(("request.v : 985 : Request->AvailSearch.holtype select <>",""))
        ...
        if clubBases = null and yachtBases = null {    
            ...
            if !rv {    
                if closedBaseList != null and !closedBaseList->IsEmpty() {    
                    let res := empty(result.Result)    
                    res->closedBaseList := closedBaseList    
call debug.DebugNL(("request.v : 1018 : ret Request->AvailSearch [<>]","_"))
                    return res    
                }
 else {    
                    raise ex.arg, msg    
call debug.DebugNL(("request.v : 1023 : ret Request->AvailSearch [<>]","_"))
                    return empty(result.Result)    
                }
            ...
            if flightRequests->required {    
                ...
                if flightCnt = 0 {    
                    ...
                    if closedBaseList != null and !closedBaseList->IsEmpty() {    
                        let res := empty(result.Result)    
                        res->closedBaseList := closedBaseList    
call debug.DebugNL(("request.v : 1077 : ret Request->AvailSearch [<>]","_"))
                        return res    
                    }
    
call debug.DebugNL(("request.v : 1081 : ret Request->AvailSearch [<>]","_"))
                    return null    
                }
...
--call promotions.Debug(Display(1))    
        ...
        gl_company_no := oldCompanyNo    
        leaway := oldLeeway    
call debug.DebugNL(("request.v : 1131 : ret Request->AvailSearch [<>]","_"))
        return res    
    }
    ...
    public function PrevDates() returns result.Result with null    
    {
call debug.DebugNL(("request.v : 1139 : f Request->PrevDates ",))
...
call debug.DebugNL(("request.v : 1150 : ret Request->PrevDates [<>]","_"))
        return prevRes    
    }
    ...
    public function NextDates() returns result.Result with null    
    {
call debug.DebugNL(("request.v : 1158 : f Request->NextDates ",))
...
call debug.DebugNL(("request.v : 1169 : ret Request->NextDates [<>]","_"))
        return nextRes    
    }
    ...
    returns result.Result with null    
    {
call debug.DebugNL(("request.v : 1178 : f Request->WidenSearch ",))
        ...
        if !validBases {    
            if closedBaseList != null and !closedBaseList->IsEmpty() {    
                let res := empty(result.Result)    
                res->closedBaseList := closedBaseList    
call debug.DebugNL(("request.v : 1204 : ret Request->WidenSearch [<>]","_"))
                return res    
            }
    
call debug.DebugNL(("request.v : 1208 : ret Request->WidenSearch [<>]","_"))
            return null    
        }
            ...
            if flightRequests->required {    
                ...
                if flightCnt = 0 {    
                    ...
                    if closedBaseList != null and !closedBaseList->IsEmpty() {    
                        let res := empty(result.Result)    
                        res->closedBaseList := closedBaseList    
call debug.DebugNL(("request.v : 1243 : ret Request->WidenSearch [<>]","_"))
                        return res    
                    }
    
call debug.DebugNL(("request.v : 1247 : ret Request->WidenSearch [<>]","_"))
                    return empty(result.Result)    
                }
        ...
        -- restore the calling company environment.    
        gl_company_no := oldCompanyNo    
call debug.DebugNL(("request.v : 1302 : ret Request->WidenSearch [<>]","_"))
        return res    
    }
    ...
    returns result.Result with null    
    {
call debug.DebugNL(("request.v : 1311 : f Request->AlternativeSearch ",))
        ...
        if res = null {    
            ...
            if leaway = 0 {    
                ...
                if res = null or !res->ValidResultSet(this) {    
                    ...
                    if res != null and res->closedBaseList != null and !res->closedBaseList->IsEmpty()    
                    {    
call debug.DebugNL(("request.v : 1333 : ret Request->AlternativeSearch [<>]","_"))
                        return res    
                    }
    
call debug.DebugNL(("request.v : 1337 : ret Request->AlternativeSearch [<>]","_"))
                    return null    
                }
            ...
            if leaway = 0 {    
                ...
                if res = null or !res->ValidResultSet(this) {    
                    ...
                    if res != null and res->closedBaseList != null and !res->closedBaseList->IsEmpty()    
                    {    
call debug.DebugNL(("request.v : 1358 : ret Request->AlternativeSearch [<>]","_"))
                        return res    
                    }
    
call debug.DebugNL(("request.v : 1362 : ret Request->AlternativeSearch [<>]","_"))
                    return null    
                }
        ...
        leaway := oldLeeway    
        call res->SetPrices(null)    
call debug.DebugNL(("request.v : 1374 : ret Request->AlternativeSearch [<>]","_"))
        return res    
    }
    ...
    returns result.Result with null    
    {
call debug.DebugNL(("request.v : 1383 : f Request->TuiGSearch [<>]","_"))
        ...
        --if res is null then check results for max pax values Leboat web only    
            ...
            call liberrmsg.LogError('avail',msg)    
                ...
                checkExpectedPax := false    
                    ...
                    if res != null and res->ValidResultSet(this) {    
                call res->SetPrices(null)    
call debug.DebugNL(("request.v : 1405 : ret Request->TuiGSearch [<>]","_"))
                return res    
            }
                ...
                --if res is null then check results for max pax values leboat web only    
                        ...
                        {    
                            ...
                            call liberrmsg.LogError('avail',msg)    
                                ...
                                if res != null and res->ValidResultSet(this) {    
                            call res->SetPrices(null)    
call debug.DebugNL(("request.v : 1459 : ret Request->TuiGSearch [<>]","_"))
                            return res    
                        }
                ...
                if res != null and res->ValidResultSet(this) {    
                    call res->SetPrices(null)    
call debug.DebugNL(("request.v : 1469 : ret Request->TuiGSearch [<>]","_"))
                    return res    
                }
            ...
            {    
                ...
                if res != null and res->ValidResultSet(this) {    
call debug.DebugNL(("request.v : 1492 : ret Request->TuiGSearch [<>]","_"))
                    return res    
                }
        ...
        leaway := oldLeeway    
        call res->SetPrices(null)    
call debug.DebugNL(("request.v : 1504 : ret Request->TuiGSearch [<>]","_"))
        return res    
    ...
    {
call debug.DebugNL(("request.v : 1515 : p Request->ProcessClubList [<>]","_"))
    
        if baseList = null {    
call debug.DebugNL(("request.v : 1518 : ret Request->ProcessClubList ",))
            return    
        }    
        if dateResultList = null or dateResultList->IsEmpty() {    
            let tmpBase := baseList->head with null    
    ...
    {
call debug.DebugNL(("request.v : 1591 : p Request->ProcessYachtList [<>]","_"))
    
            
        if baseList = null {    
call debug.DebugNL(("request.v : 1595 : ret Request->ProcessYachtList ",))
            return    
        }    
        if dateResultList = null or dateResultList->IsEmpty() {    
            let tmpBase := baseList->head with null    
    ...
    {
call debug.DebugNL(("request.v : 1670 : f Request->CheckBaseClosures [<>][<>][<>][<>][<>]",compNo,"_",startBase,"_","_"))
...
call debug.DebugNL(("request.v : 1716 : ret Request->CheckBaseClosures [<>]","_"))
        return dateRangeList    
    }
    ...
    {    
        ...
        {
call debug.DebugNL(("request.v : 1740 : f Request->ProcessBases [<>][<>]","_","_"))
...
call debug.DebugNL(("request.v : 1749 : ret Request->ProcessBases [<>]","_"))
            return baseList    
        }
    ...
    {    
        ...
        {
call debug.DebugNL(("request.v : 1839 : f Request->CheckBaseRestrictions [<>][<>]",startBase,"_"))
            ...
            {
call debug.DebugNL(("request.v : 1847 : f Request->CheckBaseRestrictions.FindRestrictions [<>][<>]",startBase,endBase))
                ...
                and F_end_base = endBase    
                {
call debug.DebugNL(("request.v : 1857 : Request->CheckBaseRestrictions.FindRestrictions.baserest select <>",""))
                ...
                if !book.check_turn_nos(accrefTpl)    
                {    
call debug.DebugNL(("request.v : 1898 : ret Request->CheckBaseRestrictions.FindRestrictions [<>]",true))
                    return true -- base is restricted    
                }
    
call debug.DebugNL(("request.v : 1902 : ret Request->CheckBaseRestrictions.FindRestrictions [<>]",isRestricted))
                return isRestricted    
            }
            ...
            if endBase in ("", null) {    
                ...
                if showAvailableBases {    
                    ...
                    and F_old_base = false    
                    {
call debug.DebugNL(("request.v : 1916 : Request->CheckBaseRestrictions.base select <>",""))
...
call debug.DebugNL(("request.v : 1938 : ret Request->CheckBaseRestrictions [<>]",allRestricted))
            return allRestricted     
        }
        ...
        {
call debug.DebugNL(("request.v : 1951 : f Request->FindBase [<>][<>][<>][<>]","_",startBase,"_","_"))
            ...
            if startBase matches "*[\*\|\?]*|$" {    
                ...
                and (F_max_dur = null or F_max_dur >= baseTravelDur)    
                {
call debug.DebugNL(("request.v : 1972 : Request->FindBase.base select <>",""))
...
--                    }    
                ...
                and (F_max_dur = null or F_max_dur >= baseTravelDur)    
                {
call debug.DebugNL(("request.v : 2033 : Request->FindBase.base select <>",""))
...
call debug.DebugNL(("request.v : 2077 : ret Request->FindBase ",))
            return (null if baseList->IsEmpty(), baseList otherwise), msg    
        }
...
--display accomNo, res->accomDateList->elemCount, fromBase, toBase    
    ...
    {    
        ...
        order by F_priority    
        {
call debug.DebugNL(("request.v : 2242 : Request->baselink select <>",""))
    ...
    {
call debug.DebugNL(("request.v : 2332 : f Request->ResultMatchYachtRequests [<>]","_"))
    
        if accomRequests = null or    
           (accomRequests->yachtRequest = null and accomRequests->waterwaysRequest = null) {    
call debug.DebugNL(("request.v : 2336 : ret Request->ResultMatchYachtRequests [<>]",true))
            return true    
        }
        ...
        if holidayType = holtype.Waterways {    
            let waterwaysRequest := accomRequests->waterwaysRequest    
            if waterwaysRequest->elite and !yacht->isPremier {    
call debug.DebugNL(("request.v : 2343 : ret Request->ResultMatchYachtRequests [<>]",false))
                return false    
            }
            ...
            let yachtRequest := accomRequests->yachtRequest    
            if yachtRequest->premierYacht and !yacht->isPremier {    
call debug.DebugNL(("request.v : 2350 : ret Request->ResultMatchYachtRequests [<>]",false))
                return false    
            }
    
        }    
call debug.DebugNL(("request.v : 2355 : ret Request->ResultMatchYachtRequests [<>]",true))
        return true    
    }
...
--                let avbkDur := avbkdt.DaysToDuration((dateRange->endDate - dateRange->startDate) as days)    
    ...
    {
call debug.DebugNL(("request.v : 3086 : f Request->ResultMatchClubRequests [<>]","_"))
    
        if accomRequests = null or accomRequests->clubRequest = null {    
call debug.DebugNL(("request.v : 3089 : ret Request->ResultMatchClubRequests [<>]",true))
            return true    
        }
    
        let clubRequest := accomRequests->clubRequest    
        if clubRequest->adultOnly and !room->adultsOnly {    
call debug.DebugNL(("request.v : 3095 : ret Request->ResultMatchClubRequests [<>]",false))
            return false    
        }
    
call debug.DebugNL(("request.v : 3099 : ret Request->ResultMatchClubRequests [<>]",true))
        return true    
    }
    ...
    {
call debug.DebugNL(("request.v : 3111 : f Request->InvalidAccomForPax [<>][<>][<>][<>]",clientNo,accomPaxCat,totalBedSpace,adultKidPax))
        ...
        where pass.F_client_no = clientNo    
        {
call debug.DebugNL(("request.v : 3120 : Request->InvalidAccomForPax.pass select <>",""))
...
call debug.DebugNL(("request.v : 3142 : ret Request->InvalidAccomForPax [<>]",invalidAccomForPax))
        return invalidAccomForPax    
    }
    ...
    {
call debug.DebugNL(("request.v : 3151 : f Request->MatchClubRequests [<>]","_"))
    
        -- Adults only checks can only be made when we have the date.    
        if accomRequests = null or accomRequests->clubRequest = null {    
call debug.DebugNL(("request.v : 3155 : ret Request->MatchClubRequests [<>]",true))
            return true    
        }
        ...
        if clubRequest->singleClubAccom {    
            ...
            if clubRequest->singles {    
                maxPax := accomTu->F_max_singles    
                if totPax > accomTu->F_max_singles {    
call debug.DebugNL(("request.v : 3167 : ret Request->MatchClubRequests [<>]",false))
                    return false    
                }
    
            } else {     
                if accomTu->F_min_sale > totPax or totPax > accomTu->F_max_sale {    
call debug.DebugNL(("request.v : 3173 : ret Request->MatchClubRequests [<>]",false))
                    return false    
                }
            ...
            if (partyClientNo != null) {    
                if InvalidAccomForPax(partyClientNo, accomTu->F_ok_child, totBeds, totPax) {    
call debug.DebugNL(("request.v : 3180 : ret Request->MatchClubRequests [<>]",false))
                    return false    
                }
    
            } else {    
                if (accomTu->F_ok_child in ("A", "F") & childPax >= 1) {    
call debug.DebugNL(("request.v : 3186 : ret Request->MatchClubRequests [<>]",false))
                    return false    
                }
        ...
        if clubRequest->balcony and accomTu->F_balcony != 'Y' {    
call debug.DebugNL(("request.v : 3193 : ret Request->MatchClubRequests [<>]",false))
            return false    
        }
    
        if clubRequest->seaView and accomTu->F_sea_view != 'Y' {    
call debug.DebugNL(("request.v : 3198 : ret Request->MatchClubRequests [<>]",false))
            return false    
        }
    
        if clubRequest->singles and accomTu->F_max_singles < 1 {    
call debug.DebugNL(("request.v : 3203 : ret Request->MatchClubRequests [<>]",false))
            return false    
        }
    
        if clubRequest->cots and accomTu->F_cots = 0 {    
call debug.DebugNL(("request.v : 3208 : ret Request->MatchClubRequests [<>]",false))
            return false    
        }
    
        if clubRequest->zbeds and accomTu->F_dbl_zbeds = 0 and    
           accomTu->F_sng_zbeds = 0 {    
call debug.DebugNL(("request.v : 3214 : ret Request->MatchClubRequests [<>]",false))
            return false    
        }
    
call debug.DebugNL(("request.v : 3218 : ret Request->MatchClubRequests [<>]",true))
        return true    
    }
    ...
    procedure RoomAvailability(    
     ...
     {
call debug.DebugNL(("request.v : 3232 : p Request->RoomAvailability [<>][<>][<>][<>][<>]","_",baseCode,"_","_","_"))
...
--                let avbkDur := avbkdt.DaysToDuration((dateRange->endDate - dateRange->startDate) as days)     
    ...
    {
call debug.DebugNL(("request.v : 3283 : p Request->BuildClubList [<>]","_"))
    
        let catAccTu := empty(schema.cat_acc)    
        let accomTu := empty(schema.accom)    
    
        if clubBases = null or clubBases->IsEmpty() {    
call debug.DebugNL(("request.v : 3289 : ret Request->BuildClubList ",))
            return    
        }    
        ...
        {
call debug.DebugNL(("request.v : 3299 : p Request->BuildClubList.ProcessRoom [<>][<>][<>]",baseCode,"_","_"))
            ...
            if doInterconnecting in (InterconnectingOnly, NoInterconnecting, StdPlusInterconnecting) {    
                ...
                and F_rel_level = "I"    
                {
call debug.DebugNL(("request.v : 3310 : Request->BuildClubList.ProcessRoom.accomrel select <>",""))
    
                    let tu := empty(schema.accom)    
                    quick select as tu from accom index F_accom_no    
                    where accom.F_accom_no = accomrel.F_rel_accom_no    
                    {
call debug.DebugNL(("request.v : 3316 : Request->BuildClubList.ProcessRoom.accomrel.accom select <>",""))
            ...
            if roomSpec != null and dateResult != null {    
                ...
                if dateResult != null and !room->IsSameRoom(roomSpec) {    
                    -- If specification of room is not the same as required, then ignore it.    
call debug.DebugNL(("request.v : 3334 : ret Request->BuildClubList.ProcessRoom ",))
                    return    
                }    
            }    
            if (doInterconnecting = AllRooms or    
        ...
        {
call debug.DebugNL(("request.v : 3369 : p Request->BuildClubList.SearchByAvail [<>][<>][<>]",baseCode,"_","_"))
            ...
            and pub.AvailAccessOk(avail.F_access)    
            on ex.lock {
call debug.DebugNL(("request.v : 3382 : Request->BuildClubList.SearchByAvail.avail select <>",""))
            ...
            {    
                ...
                and accom.F_type matches clubType    
                on ex.lock {
call debug.DebugNL(("request.v : 3393 : Request->BuildClubList.SearchByAvail.accom select <>",""))
                ...
                {    
                    if MatchClubRequests(accomTu) {    
                        if clubCat = 0 {    
                            ...
                            order by cat_acc.F_category,cat_acc.F_sort_by    
                            on ex.lock {
call debug.DebugNL(("request.v : 3408 : Request->BuildClubList.SearchByAvail.cat_acc select <>",""))
                            ...
                            order by cat_acc.F_category,cat_acc.F_sort_by    
                            on ex.lock {
call debug.DebugNL(("request.v : 3428 : Request->BuildClubList.SearchByAvail.cat_acc select <>",""))
        ...
        {
call debug.DebugNL(("request.v : 3451 : p Request->BuildClubList.SearchByCat ",))
            ...
            {
call debug.DebugNL(("request.v : 3459 : p Request->BuildClubList.SearchByCat.AvailSearch [<>][<>][<>]",accomNo,"_","_"))
                ...
                and pub.AvailAccessOk(avail.F_access)    
                on ex.lock {
call debug.DebugNL(("request.v : 3468 : Request->BuildClubList.SearchByCat.AvailSearch.avail select <>",""))
            ...
            procedure AccomSearch()    
            {
call debug.DebugNL(("request.v : 3483 : p Request->BuildClubList.SearchByCat.AccomSearch ",))
    
                quick select as accomTu from accom index F_accom_no    
                where accom.F_accom_no = catAccTu->F_accom_no    
                and accom.F_type matches clubType    
                on ex.lock {
call debug.DebugNL(("request.v : 3489 : Request->BuildClubList.SearchByCat.AccomSearch.accom select <>",""))
...
/*    
                ...
                and pub.AvailAccessOk(avail.F_access)    
                                                        ...
                                                        on ex.lock {
call debug.DebugNL(("request.v : 3559 : Request->BuildClubList.SearchByCat.AccomSearch.avail select <>",""))
...
*/    
                ...
                while compArr[compCnt] != "" {    
                    ...
                    if clubCat = 0 {    
                        ...
                        order by cat_acc.F_category,cat_acc.F_sort_by    
                        on ex.lock {
call debug.DebugNL(("request.v : 3595 : Request->BuildClubList.SearchByCat.AccomSearch.cat_acc select <>",""))
                        ...
                        order by cat_acc.F_category,cat_acc.F_sort_by    
                        on ex.lock {
call debug.DebugNL(("request.v : 3611 : Request->BuildClubList.SearchByCat.AccomSearch.cat_acc select <>",""))
    ...
    {
call debug.DebugNL(("request.v : 3713 : p Request->BuildResults [<>][<>][<>][<>][<>][<>][<>][<>][<>][<>]","_","_","_","_","_","_","_",singlesReqd,"_","_"))
...
*/    
        ...
        while tmpAccomDate != null {    
            ...
            if roomResult != null {    
                call roomResult->SetAdultsOnly(dateResult->startDate, (dateResult->startDate + avbkdt.DurationToDays(avbkDuration) days))    
                if !singlesReqd and roomResult->InvalidMinPax(adultPax + childPax) {    
call debug.DebugNL(("request.v : 3752 : ret Request->BuildResults ",))
                    return    
                }    
                if !roomResult->IsSameRoom(roomSpec) or    
                   !roomResult->IsSuitable(adultPax, childPax, infantPax) {    
                    -- If specification of room is not the same as required, then ignore it.    
call debug.DebugNL(("request.v : 3758 : ret Request->BuildResults ",))
                    return    
                }    
                if !ResultMatchClubRequests(roomResult)  {    
                    continue <<>>    
            ...
            if yachtResult != null {    
                call yachtResult->SetPremier(dateResult->startDate)    
                if !singlesReqd and yachtResult->InvalidMinPax(adultPax + childPax) {    
call debug.DebugNL(("request.v : 3768 : ret Request->BuildResults ",))
                    return    
                }    
                if !yachtResult->IsSameYacht(yachtSpec) {    
                    -- If specification of yacht is not the same as required, then ignore it.    
call debug.DebugNL(("request.v : 3773 : ret Request->BuildResults ",))
                    return    
                }    
                if !ResultMatchYachtRequests(yachtResult)  {    
                    continue <<>>    
...
--display "Rel room", relRoom->accomNo, relRoom->name, avbkdt.Date(accomDate->startDT), avbkdt.DurationToDays(avbkDuration), baseResult->startBase    
    ...
    {
call debug.DebugNL(("request.v : 3919 : p Request->ClubPromotions [<>][<>][<>][<>][<>]","_","_","_","_","_"))
    
        if !doPromotions {    
call debug.DebugNL(("request.v : 3922 : ret Request->ClubPromotions ",))
            return    
        }    
        if roomResult->status = accomres.BlueStatus {    
            let actualDateResult := promotionResultList->dateResultList->UniqueOrder(dateResult->Clone())    
    ...
    {
call debug.DebugNL(("request.v : 3945 : p Request->YachtPromotions [<>][<>][<>][<>][<>]","_","_","_","_","_"))
    
        if !doPromotions {    
call debug.DebugNL(("request.v : 3948 : ret Request->YachtPromotions ",))
            return    
        }    
        if yachtResult->status = accomres.BlueStatus {    
            let actualDateResult := promotionResultList->dateResultList->UniqueOrder(dateResult->Clone())    
    ...
    returns text    
    {
call debug.DebugNL(("request.v : 3966 : f Request->Display [<>]",indent))
...
call debug.DebugNL(("request.v : 4020 : ret Request->Display [<>]",txt))
        return txt    
    }
    ...
    {
call debug.DebugNL(("request.v : 4031 : p Request->Flights [<>][<>]","_","_"))
    
        -- Hash list to ensure that routelink searches are only performed once per airport.    
        if !((gl_company_no = "1" or gl_company_no = "7") and gl_inv_co = "1") {    
            -- flights are not applicable    
call debug.DebugNL(("request.v : 4036 : ret Request->Flights ",))
            return    
        }    
        let hashinst := empty(schash.Hash)    
        call hashinst->SetHashSize(211)    
        ...
        {
call debug.DebugNL(("request.v : 4046 : f Request->Flights.MatchesFlightRequest [<>]",route))
    
            if flightRequests = null or flightRequests->airportList = null or    
               flightRequests->airportList->IsEmpty() {    
call debug.DebugNL(("request.v : 4050 : ret Request->Flights.MatchesFlightRequest [<>]",true))
                return true    
            }
            ...
            while tmp != null {    
                let airport := flightRequests->airportList->GetAirport(tmp)    
                if route matches airport->airportCode ^ "*" {    
call debug.DebugNL(("request.v : 4058 : ret Request->Flights.MatchesFlightRequest [<>]",true))
                    return true    
                }
    
                tmp := tmp->next    
            }    
call debug.DebugNL(("request.v : 4064 : ret Request->Flights.MatchesFlightRequest [<>]",false))
            return false    
        }
        ...
        {
call debug.DebugNL(("request.v : 4078 : p Request->Flights.OutBound [<>][<>][<>][<>][<>][<>]","_","_",searchStartDate,searchEndDate,arrAirport,"_"))
            ...
            if uniqueEntry {    
                ...
                order by F_date,F_time asc    
                {
call debug.DebugNL(("request.v : 4090 : Request->Flights.OutBound.routlink select <>",""))
                    ...
                    and F_origin = gl_origin    
                    {
call debug.DebugNL(("request.v : 4104 : Request->Flights.OutBound.routlink.airorigin select <>",""))
                    ...
                    if found and MatchesFlightRequest(rTu->F_route) {    
                        ...
                        where F_route_no = rTu->F_route_no    
                        {
call debug.DebugNL(("request.v : 4117 : Request->Flights.OutBound.routlink.routflight select <>",""))
...
--display "starRoute", starRoute->Display(1)    
        ...
        procedure InBound()    
        {
call debug.DebugNL(("request.v : 4149 : p Request->Flights.InBound ",))
    
            procedure InRoute(    
                outRoute is flightres.RouteResult    
            )    
            {
call debug.DebugNL(("request.v : 4155 : p Request->Flights.InBound.InRoute [<>]","_"))
...
--display "Search", outRoute->arrAirport ^ '-' ^ outRoute->depAirport, outRoute->depNo, depDate, outRoute->starClass    
                ...
                order by F_route, F_date,F_time asc    
                {
call debug.DebugNL(("request.v : 4167 : Request->Flights.InBound.InRoute.routlink select <>",""))
    
                    select * from routflight    
                    where F_route_no = rTu->F_route_no    
                    order by F_date,F_time asc    
                    {
call debug.DebugNL(("request.v : 4173 : Request->Flights.InBound.InRoute.routlink.routflight select <>",""))
...
--                    let avbkDur := avbkdt.DaysToDuration((dateRange->endDate - dateRange->startDate) as days)     
    ...
    {
call debug.DebugNL(("request.v : 4271 : p Request->RelatedRooms [<>]","_"))
        ...
        if !room->hasRelated {    
            ...
            order by F_rel_level    
            {
call debug.DebugNL(("request.v : 4279 : Request->RelatedRooms.accomrel select <>",""))
                ...
                if (doInterconnecting in (AllRooms, NoInterconnecting) or    
                   (doInterconnecting in (InterconnectingOnly, StdPlusInterconnecting) and F_rel_level != 'I')) {    
                    ...
                    where accom.F_accom_no = accomrel.F_rel_accom_no    
                    {
call debug.DebugNL(("request.v : 4287 : Request->RelatedRooms.accomrel.accom select <>",""))
    ...
    {
call debug.DebugNL(("request.v : 4316 : p Request->AddHeldOptions [<>][<>][<>][<>][<>][<>][<>]",travelDate,"_","_","_","_","_","_"))
        ...
        if singlesReqd {    
            ...
            select one as accomTpl from accom    
                where accom.F_accom_no = accomNo {
call debug.DebugNL(("request.v : 4331 : Request->AddHeldOptions.accom select <>",""))
    ...
    {
call debug.DebugNL(("request.v : 4514 : p Request->CategoryAccommodation [<>][<>][<>][<>][<>]",travelDate,"_","_","_","_"))
        ...
        {
call debug.DebugNL(("request.v : 4521 : p Request->CategoryAccommodation.ClubCategoryAccomodation [<>][<>]","_","_"))
        ...
        {
call debug.DebugNL(("request.v : 4541 : p Request->CategoryAccommodation.YachtCategoryAccomodation [<>][<>]","_","_"))
    ...
    procedure AddHeldBookings()    
    {
call debug.DebugNL(("request.v : 4576 : p Request->AddHeldBookings ",))
        ...
        {
call debug.DebugNL(("request.v : 4586 : p Request->AddHeldBookings.ProcessCategories [<>][<>][<>][<>][<>]",processClubs,"_","_","_","_"))
        ...
        {
call debug.DebugNL(("request.v : 4612 : p Request->AddHeldBookings.ProcessBaseDates [<>][<>]","_","_"))
        ...
        proc ProcessBases(dateRes is dateres.DateResult)    
        {
call debug.DebugNL(("request.v : 4643 : p Request->AddHeldBookings.ProcessBases [<>]","_"))
...
{
call debug.DebugNL(("request.v : 4696 : f BuildRequest [<>][<>][<>][<>][<>][<>][<>][<>][<>][<>][<>][<>][<>][<>][<>][<>][<>]",startDate,travelDuration,leaway,holidayTypeCode,product,area,"_","_","_",adultPax,childPax,clubCat,clubType,yachtCat,yachtType,sourceVal,"_"))
    ...
    req->reqDateRange->startDate := ((startDate - leaway days) if leaway >= 1, startDate otherwise)    
    req->reqDateRange->endDate := ((startDate + leaway days) if leaway >= 1, startDate otherwise)    
call debug.DebugNL(("request.v : 4723 : ret BuildRequest [<>]","_"))
    return req    
}
...
returns array of string    
{
call debug.DebugNL(("request.v : 4733 : f Companies [<>]","_"))
...
call debug.DebugNL(("request.v : 4758 : ret Companies [<>]","_"))
    return companyArr    
}
